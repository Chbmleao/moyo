...
export type GetDocumentByToken = (token: string) => Promise<GetDocumentByTokenResult>;

export function makeGetDocumentByToken(
	documentRepository: DocumentRepository,
	storageRepository: StorageRepository,
): GetDocumentByToken {
	return async function getDocumentByToken(token: string): Promise<GetDocumentByTokenResult> {
