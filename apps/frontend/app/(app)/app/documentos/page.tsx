...
export default function DocumentosPage() {
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		try {
			await uploadDocument(session.access_token, file, signerEmail.trim() || null, deadline.trim() || null);
			const list = await listDocuments(session.access_token);
			setDocuments(list);
			setFile(null);
			setSignerEmail("");
		...
		<main className="min-h-[calc(100vh-4rem)] px-6 py-16 lg:px-12">
			<div className="mx-auto max-w-4xl">
				...
				<section className="mb-10 rounded-xl border border-border bg-card p-6">
					<h2 className="mb-4 text-lg font-medium text-foreground">Enviar novo documento (PDF)</h2>
					<form onSubmit={handleSubmit} className="space-y-4">
						<div>
							<label htmlFor="file-input" className="mb-1 block text-sm font-medium text-foreground">
								Arquivo PDF
							</label>
							<input
								id="file-input"
						...
						<div>
							<label htmlFor="signer_email" className="mb-1 block text-sm font-medium text-foreground">
						...
						<div>
							<label htmlFor="deadline" className="mb-1 block text-sm font-medium text-foreground">
							...
							<input
								...
								value={deadline}
								onChange={e => {
									// Append local timezone offset so the backend receives an unambiguous ISO string
				...
				<section>
					<h2 className="mb-4 text-lg font-medium text-foreground">Meus documentos</h2>
					{loading ? (
